...
def generate_qa_llama_index(
        ...
        for content in contents:
            tasks.append(
                async_qa_gen_llama_index(
                    ...
                    llm=llm,
                    prompt=prompt,
                    question_num=question_num_per_content,
    ...
    results = loops.run_until_complete(async_generate_all())
    return results
...
async def async_qa_gen_llama_index(
    ...
    async def generate(content: str, llm: LLMPredictor, delay_time: int = 4):
        while True:
            try:
...
async def generate_basic_answer(llm: LLMPredictor, passage_str: str, query: str) -> str:
